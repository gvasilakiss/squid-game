...
class Player {
    ...
    check() {
        ...
        if (!dallFacingBack && this.playerInfo.velocity > 0) {
            text.innerText = this.playerInfo.name + " You Lost🥴"
        ...
        if (this.playerInfo.positionX < end_position + .7) {
            ...
            if (SAFE_PLAYERS == players.length) {
                text.innerText = "Everyone Won, take a cookie🍪"
                gameStat = "ended"
            }
            if (DEAD_PLAYERS + SAFE_PLAYERS == players.length) {
                gameStat = "ended"
...
async function init() {
    await delay(500)
    text.innerText = "Starting in 3️⃣"
    await delay(500)
    text.innerText = "Starting in 2️⃣"
    await delay(500)
    text.innerText = "Starting in 1️⃣"
    lookBackward()
    await delay(500)
    text.innerText = "RUN🏃‍♂️"
    bgMusic.play()
    start()
}
...
function start() {
    ...
    setTimeout(() => {
        if (gameStat != "ended") {
            text.innerText = "Time Out, You died🥴"
            loseMusic.play()
            gameStat = "ended"
        }
    }, TIME_LIMIT * 1000)
